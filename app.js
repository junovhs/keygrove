...
function fingerForKey(k){if(k===' ')return{full:'Thumbs',id:'thumb'};return FINGERS.find(f=>f.keys.includes(k.toLowerCase()))||null}
function fingerText(f){
 const targets=[...f.keys],home=f.anchor,other=f.id.startsWith('l')?'j':'f',parts=[];
 for(const k of targets)parts.push(home+k+home,other);
 for(const k of [...targets].reverse())parts.push(k+home+k,other);
 return parts.join(' ')
}
...
function metrics(){
 ...
 $('wpm').textContent=w;$('acc').textContent=a+'%';$('pct').textContent=p+'%';$('combo').textContent=run.combo;return{w,a,p}
}
function focusGrid(){
...
function labels(){
 ...
 $('modeLabel').textContent=f?'Finger drill':'Trail run';
 $('lessonTitle').textContent=f?f.full:l.title;
 $('lessonCopy').textContent=f?'Practice '+f.keys.toUpperCase().split('').join(' · ')+' while returning to '+f.anchor.toUpperCase()+'.':l.copy;
...
function nextVisual(){
 document.querySelectorAll('.hand-finger,.hand-thumb,.hand-label').forEach(x=>x.classList.remove('active'));
 const c=currentChar(),f=fingerForKey(c);
 if(c===' '){
   document.querySelectorAll('.hand-thumb').forEach(x=>x.classList.add('active'));
   $('fingerCue').textContent='SPACEBAR · THUMBS';
   $('nextCue').innerHTML='<strong>PRESS SPACEBAR</strong> · it will not hurt accuracy until you do'
 }else if(f){
   document.querySelectorAll('[data-finger="'+f.id+'"]').forEach(x=>x.classList.add('active'));
   document.querySelectorAll('[data-label="'+f.id+'"]').forEach(x=>x.classList.add('active'));
   $('fingerCue').textContent=c.toUpperCase()+' · '+f.full.toUpperCase();
   $('nextCue').innerHTML='NEXT · <strong>'+escapeHtml(c.toUpperCase())+'</strong> · '+escapeHtml(f.full)
 }else{
   $('fingerCue').textContent='HANDS ON HOME ROW';$('nextCue').textContent=''
 }
}
function keymap(){
 ...
 $('keymap').innerHTML=rows.map(r=>'<div class="keyrow">'+[...r].map(k=>'<span class="keycap '+(homes.has(k)?'home ':'')+(c===k?'hot':'')+'">'+escapeHtml(k)+'</span>').join('')+'</div>').join('')+
