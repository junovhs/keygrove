...
export function objectCard(k: Keepsake, earned: boolean): string {
  ...
  return `<article class="object-card${k.holo ? ' rare' : ''}"><button type="button" class="charm-frame" data-summon="${k.id}" aria-label="Summon ${esc(k.name)}">${charmImg(k)}</button><h3>${esc(k.name)}</h3><p>${esc(k.line)}</p>`
    + (k.checkpoint ? `<button type="button" class="object-replay" data-replay="${k.checkpoint}">Replay its passage <span aria-hidden="true">↗</span></button>` : '')
    + '</article>';
}
...
export function collectionHtml(s: SaveV6): string {
  const group = (title: string, ks: readonly Keepsake[]) => `<section class="charm-group"><h3 class="eyebrow">${title} · ${ks.filter(k => owns(s, k)).length} of ${ks.length}</h3><div class="collection">${ks.map(k => objectCard(k, owns(s, k))).join('')}</div></section>`;
