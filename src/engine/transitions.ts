 ...
 */
export interface TransitionStat { err: number; lat: number; lat2: number; seen: number; hits: number }
...
export class TransitionModel {
  ...
  record(prev: string, cur: string, correct: boolean, latencyMs: number | null): void {
    ...
    if (!s) { this.stats[pair] = { err: correct ? 0 : 1, lat: timed ? lat : 0, lat2: timed ? lat * lat : 0, seen: 1, hits: correct ? 1 : 0 }; return; }
  ...
  static fromJSON(raw: unknown): TransitionModel {
    ...
    if (raw && typeof raw === 'object') {
      for (const [pair, v] of Object.entries(raw as Record<string, unknown>)) {
        ...
        stats[pair] = { err: Math.min(1, n(r.err)), lat, lat2: n(r.lat2, lat * lat), seen, hits: Math.min(seen, Math.floor(n(r.hits))) };
      }
    }
