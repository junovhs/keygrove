...
describe('createProgressSync', () => {
  ...
  it('first sign-in on a device merges the guest progress into an empty account and pushes it', async () => {
    ...
    expect(readProgressState(fake.row()?.state).save.trails[T0]!.runs).toBe(2);
    expect(sync.status()).toEqual({ kind: 'synced', runs: 2 });
  });

  it('a later sign-in adopts the account copy; sign-out restores the guest copy', async () => {
    ...
    mem.set('keygrove.sync.u1', '1'); // seen here before
    const sync = createProgressSync(local);
    sync.session(session('u1'), fake.client);
    await settle();
