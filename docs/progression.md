...
Each finger pair has ten levels, all woven into the main path as required finger stops (DEC-20). `src/curriculum/stops.ts` places each one after the earliest main lesson that teaches every character it needs, never before the pair's previous level and never more than two after one lesson. A stop holds the next lesson through Continue unless the learner has already started it. Levels 1–4 are fixed same-finger patterns (landmarks, reach up, reach down, row jumps). Levels 5–10 are generated per run and are hard on purpose: finger-heavy words, same-finger runs, twisters, capital reaches, numbers and symbols, and a gauntlet. They draw from the vocabulary plus per-pair `FINGER_TWISTERS` in `language.ts`, need 30–50 presses per side, and use only keys the course has taught. Both sides still qualify independently; the next attempt concentrates on whichever side remains unfinished. Existing finger-level credit is preserved. The course view (`src/ui/map.ts`) shows stops as chips under their lesson; there is no separate practice view.
